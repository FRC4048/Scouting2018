...
CREATE TABLE `record` (
  ...
  CONSTRAINT `fk_RECORD_REPORT1` FOREIGN KEY (`REPORT_ID`) REFERENCES `report` (`FormID`) ON DELETE CASCADE ON UPDATE CASCADE
...
INSERT INTO `record` (`ID`, `Value`, `ITEM_ID`, `REPORT_ID`) VALUES (3,'1',1,1);
...
INSERT INTO `report` (`ID`, `FormType`, `TabletNum`, `ScoutName`, `TeamNum`, `MatchNum`, `Alliance`, `FormID`) VALUES (7,1,1,'Shreya',4048,1,0,1);
...
BEGIN
    if  NOT EXISTS (SELECT LAST_INSERT_ID() FROM scouting.report) THEN 
    BEGIN
    SET @formId = 1; 
    END;
    else 
    BEGIN 
    SELECT LAST_INSERT_ID() from scouting.report into @formId; 
    SET @formId = @formId + 1; 
    END;
    end if;
    INSERT INTO scouting.report (FormID, FormType, TabletNum, ScoutName, TeamNum, MatchNum, Alliance)
	VALUES (@formId, formType, tabletNum, scoutName, teamNum, matchNum, alliance); 
    SELECT @formId INTO id;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2018-02-26 18:39:33
